...
if rclone copyto "$SERVICOS_REMOTE" "$SERVICOS_REMOTE_TMP"; then
    ...
    if [ -f "$SERVICOS_LOCAL" ]; then
        ...
        printf 'tipo_chamado;chamado;cliente_nome;telefone;whatsapp_nome;inicio;termino;servico;valor_combinado;tecnico_responsavel;data_prevista_pagamento;status;obs\ninterno;OS359;;;;;;;;;;aguardando atendimento;\n' > "$SERVICOS_REMOTE_TMP"
    fi
fi
